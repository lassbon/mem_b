...
module.exports = {
      ...
      Training.findOne({
        ...
        .then(function (training, err) {
          ...
          if (!training) {
            return res.json(404, {
              status: 'error',
              err: 'No Training with such id existing'
            })
          } else {
            Training.update(
              { id: req.param('id') },
              ...
              return res.json(200, {
                status: 'success',
                message:
                  'Training with id ' + req.param('id') + ' has been deleted'
              })
            })
          }
        })
      ...
      Training.findOne({
        ...
        .then(function (training, err) {
          ...
          if (!training) {
            ...
            Training.update({ id: req.param('id') }, req.body).exec(function (
              err,
              data
            ) {
              if (err) {
              ...
              return res.json(200, {
                status: 'success',
                message:
                  'Training with id ' + req.param('id') + ' has been updated'
              })
            })
          }
        })
  ...
  searchTrainings: function (req, res) {
    ...
    if (!req.param('searchTerm')) {
      ...
      Training.find({ title: { contains: req.param('searchTerm') } })
        .sort('createdAt DESC')
  ...
  getCompleted: function (req, res) {
    if (req.param('id')) {
      Training.findOne({ id: req.param('id'), status: 'ongoing' })
        .sort('createdAt DESC')
        .then(function (training, err) {
          if (err) {
            sails.log.error(err)
      ...
      Training.find({ status: 'completed' })
        .sort('createdAt DESC')
        .then(function (training, err) {
          if (err) {
            sails.log.error(err)
  ...
  getTrainings: function (req, res) {
    if (req.param('id')) {
      Training.findOne({ id: req.param('id') })
        .sort('createdAt DESC')
        .then(function (training, err) {
          if (err) {
            sails.log.error(err)
