...
module.exports = {
  ...
  oldMemberLogin: (req, res) => {
    ...
    if (!membershipId || !password) {
      return res.json(401, {
        status: 'error',
        err: 'membershipId and password required'
      })
    }

    User.findOne({ membershipId: req.body.membershipId })
      .then((user, err) => {
        if (!user) {
          return res.json(401, {
            status: 'error',
            err: 'invalid membershipId or password'
          })
        }
  ...
  adminLogin: (req, res) => {
    ...
    Admin.findOne({ username: username })
      .then((admin, err) => {
        ...
        Admin.comparePassword(password, admin, (err, valid) => {
          if (err) {
            sails.log.error(err)
            return res.json(403, { status: 'error', err: 'forbidden' })
          }
