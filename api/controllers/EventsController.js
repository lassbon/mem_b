...
module.exports = {
      ...
      Events.findOne({
        ...
        .then(function(event, err) {
          ...
          if (!event) {
            return res.json(404, {
              status: "error",
              err: "No Event with such id existing"
            });
          } else {
            Events.update(
              { id: req.param("id") },
              ...
              return res.json(200, {
                status: "success",
                message:
                  "Event with id " + req.param("id") + " has been deleted"
              });
            });
          }
        })
      ...
      Events.findOne({
        ...
        .then(function(event, err) {
          ...
          if (!event) {
            ...
            Events.update({ id: req.param("id") }, req.body).exec(function(
              err,
              data
            ) {
              if (err) {
              ...
              return res.json(200, {
                status: "success",
                message:
                  "Event with id " + req.param("id") + " has been updated"
              });
            });
          }
        })
        ...
        .then(function(event, err) {
          ...
          if (!event) {
            ...
            if (stat == false) {
              return res.json(403, {
                status: "error",
                err: "Event already liked"
              });
            } else {
              likes.push(req.param("liker"));

              Events.update({ id: req.param("id") }, { likes: likes }).exec(
                function(err, event) {
                  return res.json(200, {
                    status: "success",
                    message: "Event liked"
        ...
        .then(function(event, err) {
          ...
          if (!event) {
            ...
            if (stat == false) {
              return res.json(403, {
                status: "error",
                err: "Event not previously liked"
              });
            } else {
  ...
  searchEvents: function(req, res) {
    ...
    if (!req.param("searchTerm")) {
      ...
      Events.find({ title: { contains: req.param("searchTerm") } })
        .sort("createdAt DESC")
  ...
  getCompleted: function(req, res) {
    if (req.param("id")) {
      Events.findOne({ id: req.param("id"), status: "completed" })
        .sort("createdAt DESC")
        .then(function(event, err) {
          if (err) {
            sails.log.error(err);
  ...
  getOngoing: function(req, res) {
    if (req.param("id")) {
      Events.findOne({ id: req.param("id"), status: "ongoing" })
        .sort("createdAt DESC")
        .then(function(event, err) {
          if (err) {
            sails.log.error(err);
  ...
  myEvents: function(req, res) {
    ...
    EventPayments.find({ payer: req.param("id") })
      .sort("createdAt DESC")
      .then(function(events, err) {
        if (err) {
          sails.log.error(err);
  ...
  getEvents: function(req, res) {
    if (req.param("id")) {
      Events.findOne({ id: req.param("id") })
      ...
      EventComments.findOne({
        ...
        .then(function(comment, err) {
          ...
          if (!comment) {
            ...
            EventComments.update({ id: req.param("id") }, req.body).exec(
              function(err, data) {
                if (err) {
                  sails.log.error(err);
                  return res.json(err.status, { err: err });
      ...
      EventComments.findOne({
        ...
        .then(function(comment, err) {
          ...
          if (!comment) {
            ...
            EventComments.destroy({ id: req.param("id") }, req.body).exec(
              function(err, data) {
                if (err) {
                  sails.log.error(err);
                  return res.json(err.status, { err: err });
  ...
  getComment: function(req, res) {
    if (req.param("id")) {
      EventComments.findOne({ id: req.param("id") })
        .sort("createdAt DESC")
        .then(function(comment, err) {
          if (err) {
            sails.log.error(err);
