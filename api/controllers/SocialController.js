    ...
    User.findOne({
      ...
      .then(function(user, err) {
        ...
        SocialPosts.create(req.body).exec(function(err, post) {
          ...
          // Handle postImage image uploads
          if (req.param("postImage")) {
            azureBlob.upload(
              container,
              req.param("postImage"),
              azureResponse => {
    ...
    SocialPosts.findOne({
      ...
      .then(function(post, err) {
        ...
        if (post.postImage && post.postImage !== req.param("postImage")) {
          ...
          azureBlob.delete(container, image.split("/").reverse()[0]);
        }

        // Handle postImage image uploads
        if (req.param("postImage")) {
          azureBlob.upload(container, req.param("postImage"), azureResponse => {
            SocialPosts.update(
              { id: req.param("id") },
              { postImage: azureResponse }
            ).exec((err, data) => {
              ...
              if (user.postImage && user.postImage !== data.postImage) {
                var url = user.postImage;
