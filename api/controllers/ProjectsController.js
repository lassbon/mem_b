...
module.exports = {
      ...
      Projects.findOne({
        ...
        .then(function(project, err) {
          ...
          if (!project) {
            return res.json(404, {
              status: "error",
              err: "No Project with such id existing"
            });
          } else {
            Projects.update(
              { id: req.param("id") },
              ...
              return res.json(200, {
                status: "success",
                message:
                  "Project with id " + req.param("id") + " has been deleted"
              });
            });
          }
        })
      ...
      Projects.findOne({
        ...
        .then(function(project, err) {
          ...
          if (!project) {
            ...
            Projects.update({ id: req.param("id") }, req.body).exec(function(
              err,
              data
            ) {
              if (err) {
              ...
              return res.json(200, {
                status: "success",
                message:
                  "Project with id " + req.param("id") + " has been updated"
              });
            });
          }
        })
  ...
  searchProjects: function(req, res) {
    ...
    if (!req.param("searchTerm")) {
      ...
      Projects.find({ title: { contains: req.param("searchTerm") } })
        .sort("createdAt DESC")
  ...
  getProjects: function(req, res) {
    if (req.param("id")) {
      Projects.findOne({ id: req.param("id") })
      ...
      ProjectComments.findOne({
        ...
        .then(function(comment, err) {
          ...
          if (!comment) {
            ...
            ProjectComments.update({ id: req.param("id") }, req.body).exec(
              function(err, data) {
                if (err) {
                  sails.log.error(err);
                  return res.json(err.status, { err: err });
      ...
      ProjectComments.findOne({
        ...
        .then(function(comment, err) {
          ...
          if (!comment) {
            ...
            ProjectComments.destroy({ id: req.param("id") }, req.body).exec(
              function(err, data) {
                if (err) {
                  sails.log.error(err);
                  return res.json(err.status, { err: err });
  ...
  getComment: function(req, res) {
    if (req.param("id")) {
      ProjectComments.findOne({ id: req.param("id") })
        .sort("createdAt DESC")
        .then(function(comment, err) {
          if (err) {
            sails.log.error(err);
        ...
        .then(function(project, err) {
          ...
          if (!project) {
            ...
            if (stat == false) {
              return res.json(403, {
                status: "error",
                err: "Project already liked"
              });
            } else {
              likes.push(req.param("liker"));

              Projects.update({ id: req.param("id") }, { likes: likes }).exec(
        ...
        .then(function(project, err) {
          ...
          if (!project) {
            ...
            if (stat == false) {
              return res.json(403, {
                status: "error",
                err: "Project not previously liked"
              });
            } else {
