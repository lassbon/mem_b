        ...
        if (hash == req.headers['x-paystack-signature']) {
            ...
            switch (event.event) {
                ...
                case 'subscription.create':
                    ...
                    User.findOne({ select: ['companyName', 'membershipId'], where: { email: event.data.customer.email } }).then(function(user, err) {
                            if (err) {
                                sails.log.error(err);
                            }
                ...
                case 'charge.success':
                    ...
                    if (event.data.plan.id) {
                        ...
                        User.findOne({ select: ['membershipId', 'companyName'], where: { email: event.data.customer.email } }).the(function(user, err) {
                                if (err) {
                                    sails.log.error(err);
                                }
